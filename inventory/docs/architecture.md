...
- **Размещение (`occupancy_options`)** перечисляет продаваемые комбинации
  гостей `(adults, дети по возрастным бэндам)` с готовым `occ_key`. Цена
  задаётся на тариф × размещение, поэтому запрос под конкретный состав гостей
  (включая **возраст детей**) резолвится в один `occupancy_id`. См. §3b.
...
### 3b. Возраст детей в поиске и цене

Цена зависит от **возраста** детей (0-1 бесплатно, 2-6, 7-12, 13-17 — разные
доли от взрослого тарифа). Клиент указывает возраст (напр. дети 6 и 8), и
цена должна считаться именно под него.

Чтобы это работало при поиске сотен отелей одним сканом, возрастные группы —
**глобальные (платформенные)**, таблица `child_age_bands`. Детские политики
отелей/контрактов маппятся в эти бэнды при загрузке (PHP). Тогда:

- **`occ_key`** кодирует не «сколько детей», а **состав по бэндам**:
  `adults*1000 + b1*100 + b2*10 + b3` (b1..b3 — число детей в каждом
  тарифицируемом бэнде). Он глобальный → одинаков во всех отелях.
- **Резолв запроса (PHP):** каждый возраст → бэнд (`child_age_bands`) →
  счётчики по бэндам → `occ_key`. Пример: `2 взр + дети 6,8` →
  `b1(2-6)=1, b2(7-12)=1` → **occ_key 2110**; `2 взр + дети 15,16` →
  `b3(13-17)=2` → **occ_key 2002** — это разные строки с разной ценой.
- **Инфанты (0-1)** — отдельно (`room.max_infants`, `in_occupancy=0`), в
  `occ_key` не входят.
- **Скорость сохранена:** возраст — это лишь другое значение `:occ`, тот же
  индекс `idx_hotel`/`idx_city` (проверено: index-only, доли мс). Цена уже
  посчитана под возрастной состав на этапе пересборки.

> **Компромисс:** бэнды платформенные, а не пер-отельные. Отель с политикой
> «ребёнок до 11» маппится в ближайший глобальный бэнд при загрузке. Это
> плата за многоотельный поиск одним `:occ`; пер-отельные бэнды потребовали
> бы досчёта цены по каждому отелю в рантайме и убили бы скорость.
