...
### 3b. Возраст детей: база в SQL, детская цена в PHP

Цена зависит от **возраста** детей, а границы возрастов у отелей **разные**.
Чтобы возрасты остались точными и пер-отельными, но поиск сотен отелей — быстрым,
...
2. **Детская доплата по возрасту** — компактный **пер-отельный** конфиг:
   - `child_age_bands` — возрастные бэнды отеля (`hotel_id`; `NULL` = дефолт);
   - `child_prices` — цена за ребёнка по бэнду/периоду/тарифу
     (`free` / `percent` от взрослой ночи / `fixed`).
   В горячий скан **не входит**.

**Поток поиска:**
- **SQL** (быстрый, index-only) считает БАЗУ по взрослым и фильтрует
  **вместимость** по счётчикам (`max_occupancy/max_adults/max_children/
  max_infants` — денормализованы в `search_daily`, возраст не нужен).
  Возвращает **набор кандидатов** — по каждому `(hotel, rate_plan)`.
- **PHP** для точных возрастов: возраст → бэнд отеля (`child_age_bands`) →
  `child_prices` → доплата за ночи; прибавляет к базе → полная цена; затем
  `MIN` по отелю. Min в PHP потому, что детская доплата может поменять, какой
  тариф дешевле.

**Почему min уезжает в PHP, а не в SQL:** без детей `MIN` по отелю берёт сам
SQL (запросы B/C как есть). С детьми внешний `MIN` снимаем и отдаём
per-`(hotel, rate_plan)` в PHP — он финализирует цену с детьми и минимизирует.
Строк немного (сотни отелей × единицы тарифов = тысячи), арифметика тривиальна.

- **Инфанты (0-1)** — бесплатно, вместимость через `room.max_infants`
  (`in_pricing = 0`), в цену не идут.
- **Скорость:** SQL остаётся тем же index-only сканом по `idx_hotel`/`idx_city`
  (occupancy = adults). Детская математика — на маленьком наборе кандидатов.

**Крайние случаи «нет цены на ребёнка»:**
- **Отель не размещает с детьми** (`room.max_children = 0`) → отсекается
  SQL-фильтром вместимости `max_children >= :children`; в кандидаты не попадает.
- **Размещает, но нет `child_prices` для возраста** → PHP **исключает тариф**
  (не показываем недосчитанную цену). Если так отпали все тарифы отеля — отель
  не попадает в выдачу. Безопаснее не показать, чем продать ниже контракта.
- Политика на этот случай конфигурируема: `reject` (дефолт) или `as_adult`
  (считать ребёнка как доп. взрослого) — решается в PHP. Пример:
  `php/child_pricing_example.php`.

> **Плюс подхода:** возрасты **пер-отельные и точные** (никаких платформенных
> округлений), и кэш **меньше** (нет строк на каждую детскую комбинацию).
> Цена за это — финальный `MIN` считает PHP, а не SQL; для «сотен отелей» это
> тысячи строк и незаметно.
