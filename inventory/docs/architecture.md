...
**Крайние случаи «нет цены на ребёнка»:**
- **Не принимает детей** (`hotels.allow_children = 0` или `room.max_children
  = 0`, либо возраст `< children_min_age`) → отсекается фильтром вместимости
  и проверкой политики; в кандидаты не попадает.
- **Принимает, но нет диапазона `child_rates` на этот возраст** → PHP
  **исключает тариф** (не показываем недосчитанную цену); если так отпали все
  тарифы — отель не в выдаче. Политика конфигурируема: `reject` (дефолт) или
  `as_adult`. Пример: `php/child_pricing_example.php`.
