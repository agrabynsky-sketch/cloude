...
--     Обновляем только если пришедшая ревизия не старше сохранённой и
--     подключению разрешено управлять наличием (проверяется в PHP).
...
-- (c) Поставить задачу на пересборку кэша по затронутому диапазону.
