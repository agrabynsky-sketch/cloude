...
-- Удаляет только то, что попадает в диапазоны id из search_demo_registry,
...
-- или целиком: TRUNCATE TABLE unit_search.search_stay (если там только демо).
-- =====================================================================

DROP PROCEDURE IF EXISTS search_demo_cleanup;

DELIMITER $$

CREATE PROCEDURE search_demo_cleanup()
BEGIN
  ...
  SELECT id_from, id_to INTO v_h1, v_h2   FROM search_demo_registry WHERE entity = 'hotels';
  SELECT id_from, id_to INTO v_r1, v_r2   FROM search_demo_registry WHERE entity = 'hotels_rooms';
  SELECT id_from, id_to INTO v_t1, v_t2   FROM search_demo_registry WHERE entity = 'hotels_rates';
  SELECT id_from, id_to INTO v_rr1, v_rr2 FROM search_demo_registry WHERE entity = 'hotels_rates_rooms';
  ...
  DELETE FROM search_demo_registry;

  SELECT 'demo data removed' AS status, v_h1 AS first_hotel_id, v_h2 AS last_hotel_id;
END$$

DELIMITER ;

CALL search_demo_cleanup();
