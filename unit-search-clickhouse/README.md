...
**Имена.** Все новые таблицы — с префиксом `hotels_` (`hotels_search_stay`, `hotels_search_sync_queue`, `hotels_search_demo_registry`, staging `hotels_search_stg_*`), все поля-идентификаторы — `id_*`, как в ваших таблицах: `id_hotel`, `id_rate_room`, `id_room`, `id_rate`, `id_parent`, `id_country`, `id_region`, `id_city`, `id_currency`, `id_board_type`, `id_cancel_policy`, `id_room_type`, `id_access_group` (из `hotels_rates.access_group_id`). Так же названы ключи параметров поиска и ответов моделей.
...
CH="clickhouse-client --user search_writer --password ..." clickhouse/full_load.sh   # силами ClickHouse, ~1 мин
...
// array('total' => 492, 'items' => array(array('id_hotel' => 1554, 'price' => 9264, 'price_minor' => 926400,
//        'id_currency' => 46688, 'stars' => 4, 'id_rate_room' => 12010, 'id_room' => 3756, 'id_rate' => 3259,
//        'id_board_type' => 1, 'id_cancel_policy' => 0, 'refundable' => 0, 'nights' => 7), ...))

$rates = Search_Model_Stay::getInstance()->hotelRates(1554, array('checkin' => '2026-12-10', 'nights' => 7, 'guests' => 2));
// array(array('id_rate_room' => .., 'id_room' => .., 'id_rate' => .., 'id_parent' => .., 'id_board_type' => ..,
//        'id_cancel_policy' => .., 'refundable' => .., 'id_room_type' => .., 'max_guests' => .., 'id_currency' => .., 'rooms_left' => 4,
...
| `search()` 1000 `id_hotel` | 21–22 мс | 25 мс |
...
2. Создать `hotels_search_sync_queue`; по желанию применить `00_recommended_indexes.sql` (через pt-osc/gh-ost).
